...
export class CvHLines {
    ...
    paint() {
        ...
        for (let j = 0; j < nFaces; j++) {
            //Polygon3D pol = (Polygon3D)(polyList.elementAt(j));
            let pol = polyList[j];
            let t = pol.getT(); // Triangles of one polygon
            if (pol.getNrs().length > 2 && pol.getH() <= this.hLimit) {
    ...
    buildLineSet() {
        ...
        for (let j = 0; j < nFaces; j++) {
            //Polygon3D pol = (Polygon3D)(polyList.elementAt(j));
            let pol = this.polyList[j]; //TERCERA REFERENCIA A POLYLIST
            let nrs = pol.getNrs();
            let n = nrs.length;
            if (n > 2 && pol.getH() > 0)
                continue;
            ...
            for (let k = 0; k < n; k++) {
                ...
                if (jj < 0)
                    ...
                    let i1 = Math.min(ii, jj), j1 = Math.max(ii, jj), nCon = this.nConnect[i1];
    ...
    lineSegment(g, p, q, pScr, qScr, iP, iQ, iStart) {
        ...
        for (let i = iStart; i < this.nTria; i++) {
            ...
            let pol = this.polyList[iPol]; ////REFERENCIA A POLYLIST GLOBAL
            let a = pol.getA(), b = pol.getB(), c = pol.getC(), h = pol.getH(), eps1 = 1e-5 * Math.abs(h), hP = a * p.x + b * p.y + c * p.z, hQ = a * q.x + b * q.y + c * q.z;
            if (hP > h - eps1 && hQ > h - eps1)
                continue; // This triangle does not obscure PQ.
            // 7. Test if both P and Q behind triangle ABC:
            let pInside = Tools2D.insideTriangle(aScr, bScr, cScr, pScr);
            let qInside = Tools2D.insideTriangle(aScr, bScr, cScr, qScr);
            if (pInside && qInside)
            ...
            for (let ii = 0; ii < 3; ii++) {
                let v1 = bScr.x - aScr.x, v2 = bScr.y - aScr.y, w1 = aScr.x - pScr.x, w2 = aScr.y - pScr.y, denom = u2 * v1 - u1 * v2;
                if (denom != 0) {
